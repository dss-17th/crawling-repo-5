...
def crawlingquery(symbol):
    ...
    url = "https://investing.com/search/?q={}".format(symbol)
...
def crawling_investing(query):
    try:
        url = "https://kr.investing.com/equities/{}".format(query)
        headers = {"User-Agent": "Mozilla/5.0"}
        response = requests.get(url, headers=headers)
        dom = BeautifulSoup(response.content, "html.parser")
...
def crawling_investing2(query):
    ...
    try:
        url = "https://kr.investing.com/equities/{}-financial-summary".format(query)
        headers = {
...
for row in querys.values:
    ...
    url = "https://kr.investing.com/equities/{}-news".format(query)
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36"}
    response = requests.get(url, headers=headers)
    if response.status_code >= 300:
        ...
        response = requests.get(url, headers=headers)
        if response.status_code >= 300:
            print('{}의 서버가 응답하지 않습니다. 에러 코드 {response.status_code}'.format(symbol))
